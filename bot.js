...
function respond() {
  var request = JSON.parse(this.req.chunks[0]),
      botRegex = /^\/cool guy/;  botRegexDL = /^\/DDL/i;botRegexSalt = /^\/salt/;botCommands = /^\/commands/;
      ...
      botRegexDaf = /^\/dafuq/; botRegexMA32 = /^\/pending/; botRegexTrade = /^\/trade/; botRegexStats = /^\/stats/; 
      botRegexStandings = /^\/standings/; botRegexScores = /^\/scores/; jags = /^\/jags/; jagsbearsraiders = /^\/jagsbearsraiders/; gif = /^\/niners/;
  ...
  if(request.text && botRegex.test(request.text)) {
    ...
    this.res.writeHead(200);
    postMessage("https://i.imgur.com/XP75ydu.gif");
    this.res.end();
  }
    ...
    this.res.writeHead(200);
    postMessage("https://i.imgur.com/OaXMT.gif");
    this.res.end();
  }
  else if(request.text && botRegexScores.test(request.text)) {
    this.res.writeHead(200);
    postMessage("https://www.daddyleagues.com/ruthless/schedules");
    this.res.end();
  }
  else if(request.text && botRegexStats.test(request.text)) {
    this.res.writeHead(200);
    postMessage("https://www.daddyleagues.com/ruthless/stats/player");
    this.res.end();
  } 
  else if(request.text && botRegexStandings.test(request.text)) {
    this.res.writeHead(200);
    postMessage("https://www.daddyleagues.com/ruthless/standings");
    this.res.end();
  } 
    ...
    postMessage("http://daddyleagues.com/ruthless/team/"+request.text.substring(5,8)+"/depthchart");
    this.res.end();
  } 
    ...
    postMessage("http://daddyleagues.com/ruthless/team/"+request.text.substring(5,8)+"/schedule");
    this.res.end();
  }
    ...
    var rep = req.replace(/ /,"+");
    postMessage("http://daddyleagues.com/ruthless/players?name="+rep+"&position=all&team=all");
